...
\begin{prop}
    ...
    \begin{proof}
        Suppose that $f(X)$ is not connected, then $\exists$ a separation $U, V$ of $f(X)$, i.e. exists nonempty open sets $U, V$ in $Y$ such that $U\cap V\cap f(X) = \emptyset$ and $U\cup V \supset X$. Then $f^{-1}(U)$ and $f^{-1}(V)$ are disjoint nonempty open sets in $X$ such that $X = f^{-1}(U) \cup f^{-1}(V)$, contradicting the connectedness of $X$. Therefore, $f(X)$ is connected.
    \end{proof}
\end{prop}
